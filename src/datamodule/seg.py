...
def load_features(
    ...
    for series_id in series_ids:
        ...
        features[series_dir.name] = np.stack(this_feature, axis=1) # {'series_id: (duration,num_features)'}
...
def load_chunk_features(
    ...
    for series_id in series_ids:
        series_dir = processed_dir / phase / series_id
        this_feature = [] # (num_timestamps, num_features)
        for feature_name in feature_names:
            this_feature.append(np.load(series_dir / f"{feature_name}.npy"))
        this_feature = np.stack(this_feature, axis=1)
        num_chunks = (len(this_feature) // duration) + 1
        for i in range(num_chunks):
            chunk_feature = this_feature[i * duration : (i + 1) * duration] # (duration, num_features)
            chunk_feature = pad_if_needed(chunk_feature, duration, pad_value=0)  # type: ignore
            features[f"{series_id}_{i:07}"] = chunk_feature # {key: (duration,num_features)}
...
# Augmentation
###################
def random_crop(pos: int, duration: int, max_end) :
    """Randomly crops with duration length including pos. 随机裁剪包含pos在内的duration的时间片段
...
def get_label(
    ...
    this_event_df = this_event_df.query("@start <= wakeup & onset <= @end") # 只选在duration内发生的事件
    # 将duration内对应的事件设为为1,其他都是awake状态(1标识睡觉状态)
    label = np.zeros((num_frames, 3)) # (2880, 3) [awake, onset, wakeup]
    # onset, wakeup, sleepのラベルを作成
    for onset, wakeup in this_event_df[["onset", "wakeup"]].to_numpy(): # 遍历duration内发生的每对时间
...
def gaussian_label(label: np.ndarray, offset: int, sigma: int) -> np.ndarray:
    num_events = label.shape[1] # 标签类型个数
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pos = self.event_df.at[idx, event] # 当前随机选取的事件所处时间步
        series_id = self.event_df.at[idx, "series_id"] # 获取记录当前事件的加速度计
        ...
        n_steps = this_feature.shape[0] # 当前加速度计记录数据的时间线长度
        ...
        feature = this_feature[start:end]  # (duration, num_features) 获取截取的duration时间p

        # upsample
        feature = torch.FloatTensor(feature.T).unsqueeze(0)  # (1, 4, 5750) 5760 * 5s = 8h * 60min * 60s
        feature = resize(
            feature,
        ...
        num_frames = self.upsampled_num_frames // self.cfg.downsample_rate # 5760//2 =2880
...
class ValidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        key = self.keys[idx]
        feature = self.chunk_features[key] # (duration, num_features)
        feature = torch.FloatTensor(feature.T).unsqueeze(0)  # (1, num_features, duration)
        feature = resize(
            feature,
            size=[self.num_features, self.upsampled_num_frames],
            antialias=False,
        ).squeeze(0) # (num_features, duration)
...
class SegDataModule(LightningDataModule):
    def __init__(self, cfg: DictConfig):
        ...
        self.valid_chunk_features = load_chunk_features(
            ...
            processed_dir=self.processed_dir,
            phase="train",
        ) # {key1:{duration,num_features}, keys2: {duration,num_features},...}
