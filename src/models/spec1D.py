...
class Spec1D(nn.Module):
    ...
    def forward(
        ...
        x = x.transpose(1, 3)  # (batch_size, n_timesteps, height, n_channels) [32, 2880, 64, 3]
        x = self.channels_fc(x)  # (batch_size, n_timesteps, height, 1) [32,2880,64,1]
        x = x.squeeze(-1).transpose(1, 2)  # (batch_size, height, n_timesteps) [32, 2880, 64]
        logits = self.decoder(x)  # (batch_size, n_classes, n_timesteps) # [32, 2880, 3]
