...
class SAKTResidualTransformer(nn.Module):
    def __init__(self,
                 in_channels: int,
                 embed_dim: int,
                 nlayers: int,
        ...
        conv_layers = [
            nn.Conv1d(embed_dim, embed_dim, (nlayers - i) * 2 - 1, stride=1, padding=((nlayers - i) * 2 - 1 - 1) // 2)
            for i in range(nlayers)
        ]
        layer_norm_layers = [
    ...
    def forward(self, x):
        ...
        x = self.layer_norm(x) # [32, 2880, 128]



        for conv, transformer_layer, layer_norm1, in zip(self.conv_layers,
