...
class SegModel(LightningModule):
    def __init__(
        ...
        self.model = get_model(
            ...
            n_classes=num_classes,  # 3
            num_timesteps=num_timesteps // cfg.downsample_rate,  # 2880
        )
    ...
    def on_validation_epoch_end(self):
        ...
        loss = losses.mean()
        bs, duration, num_classes = preds.shape
        for i in range(bs):
            awake_prob = preds[i, :, 0]
            onset_prob = preds[i, :, 1]
            wakeup_prob = preds[i, :, 2]
            if np.max(awake_prob) == np.max(preds[i]):
                preds[i, :, :] = 0
