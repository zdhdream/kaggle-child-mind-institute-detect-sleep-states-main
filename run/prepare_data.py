...
def add_feature(series_df: pl.DataFrame) -> pl.DataFrame:
    series_df = series_df.with_columns(
        ...
        *to_coord(pl.col("timestamp").dt.day(), 30, "day"),
        pl.col("anglez").diff().abs().over("series_id").alias(f"abs_anglez_diff"),
    ).select("series_id", *FEATURE_NAMES)
    return series_df


def save_each_series(this_series_df, columns, output_dir):
...
def main(cfg: DictConfig):
    ...
    with trace("Save features"):
        for series_id, this_series_df in tqdm(series_df.group_by("series_id"), total=n_unique):
            ...
            this_series_df = this_series_df.fill_null(0)
